#--------------------------------------------------------------------------
# F_runMCMC.R
# Leontine Alkema, Jin Rou New and Niamh Cahill 
#--------------------------------------------------------------------------
RunMCMC <- function(# Start MCMC sampling
  ...
  ChainNums <- unique(ChainNums) ###ChainNums Argument 6
  ...
  ##Ouput Directory Argument 15. NULL by default
  ...
  ###Creates a temporary file for JAGSobjects
...
GetBugsData <- function( # Construct winbugs.data object
  ...
  if (do.country.specific.targets.run) {
    ...
    totestyears<-seq(1950.5,2015.5) #Change NC, 20160807 
    ...
    for(c in 1:C){
      ...
      year.begin.c[c]<-min(1985.5,min(gett.j[select])) ##Need at least a few years before start point
      year.begin.index[c]<-which(totestyears==year.begin.c[c])
      year.end.c[c]<-max(1995.5,max(gett.j[select])) ##need at least a few years after start point
  ...
  priorspecs <- GetBugsPriorSpecs(change.priors.to.zerolower = change.priors.to.zerolower,
                                  ...
                                  data.global = data.global# change JR, 20131104 #Temporary Change NC, 20160807
                                  ) 
