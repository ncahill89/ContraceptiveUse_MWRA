...
InternalGetInfoOnChange <- function(# Find info on a change
  ...
  res <- c(quantile(change.s, percentiles), mean(change.s>0)) 
  names(res)[6] <- "PPPC"
...
GetInfoChange <- function(# Find info on a change in an indicator
  ...
  if (type.is.prop){ # add met demand
    P.Lcat.ts[["Met Demand"]] <- (P.yp3s[,1,]+P.yp3s[,2,])/
      (P.yp3s[,1,]+P.yp3s[,2,]+P.yp3s[,3,])
  }
  years.all <- as.numeric(dimnames(P.yp3s)[[1]])
  nchanges <- nrow(years.change) + nrow(years.change2)
  change.Lcat.Ti <- list()
  for (catname in names(P.Lcat.ts)){
