...
PlotComparison <- function(# Plot lots of results!
  ...
  ... ##<< Other arguments for \code{PlotDataAndEstimates}.
){
  if (is.null(fig.dir))
  ...
  if (!is.null(run.name4)) {
    load(file = paste(output.dir4,"res.country.rda", sep = ""))
    res.country4 <- res.country
    rm(res.country)
    if (plot.for.aggregates) {
      load(file = paste(output.dir4,"res.aggregate.rda", sep = ""))
      res.aggregate4 <- res.aggregate
      rm(res.aggregate)
    }
  }
  if (!is.null(run.name3)) {
    load(file = paste(output.dir3,"res.country.rda", sep = ""))
    res.country3 <- res.country
    rm(res.country)
    if (plot.for.aggregates) {
      load(file = paste(output.dir3,"res.aggregate.rda", sep = ""))
      res.aggregate3 <- res.aggregate
      rm(res.aggregate)
    }
  }
  ...
  rm(res.country)
  if (plot.for.aggregates) {
    load(file = paste(output.dir2,"res.aggregate.rda", sep = ""))
    res.aggregate2 <- res.aggregate
    rm(res.aggregate)
  }
  load(file = paste(output.dir,"mcmc.meta.rda", sep = ""))
  load(file = paste(output.dir,"res.country.rda", sep = ""))
  if (plot.for.aggregates)
    load(file = paste(output.dir,"res.aggregate.rda", sep = ""))
  ...
  if (!is.null(run.name4)) {
    iso.order4 <- match(res.country$iso.g, res.country4$iso.g)
  ...
  if (!is.null(run.name3)) {
    iso.order3 <- match(res.country$iso.g, res.country3$iso.g)
  ...
  iso.order2 <- match(res.country$iso.g, res.country2$iso.g)
  ...
  PlotDataAndEstimates(data.raw = mcmc.meta$data.raw, 
                       ...
                       turn.legend.off = turn.legend.off,
                       fig.name = file.path(fig.dir, paste0(run.name, " vs ", run.name2, 
                                                            ifelse(is.null(run.name3), "", paste0(" vs ", run.name3)),
                                                            ifelse(is.null(run.name4), "", paste0(" vs ", run.name4)), "CIs_nopar", fig.name.years, ".pdf")),
                       ...)
  ...
  if (plot.ind.country.results) {
    PlotDataAndEstimates(data.raw = mcmc.meta$data.raw, 
                         ...
                         fig.name = paste0(run.name, " vs ", run.name2,
                                           ifelse(is.null(run.name3), "", paste0(" vs ", run.name3)),
                                           ifelse(is.null(run.name4), "", paste0(" vs ", run.name4)), fig.name.years),
                         ...)
  }
  ...
  PlotDataAndEstimates(
    ...
    fig.name = file.path(fig.dir, paste0(run.name, " vs ", run.name2, 
                                         ifelse(is.null(run.name3), "", paste0(" vs ", run.name3)), 
                                         ifelse(is.null(run.name4), "", paste0(" vs ", run.name4)), "CIscountcountry", fig.name.years, ".pdf")),
    ...)
  ...
  if (plot.for.aggregates) {    
    PlotDataAndEstimates(CI.Lg.Lcat.qt = res.aggregate$CIprop.Lg.Lcat.qt,
                         ...
                         fig.name = file.path(fig.dir, paste0(run.name, " vs ", run.name2, 
                                                              ifelse(is.null(run.name3), "", paste0(" vs ", run.name3)), 
                                                              ifelse(is.null(run.name4), "", paste0(" vs ", run.name4)), "CIsaggregate", fig.name.years, ".pdf")),
                         ...)
    PlotDataAndEstimates(
      ...
      fig.name = file.path(fig.dir, paste0(run.name," vs ", run.name2, 
                                           ifelse(is.null(run.name3), "", paste0(" vs ", run.name3)), 
                                           ifelse(is.null(run.name4), "", paste0(" vs ", run.name4)), "CIscountaggregate", fig.name.years, ".pdf")),
      ...)
  }
