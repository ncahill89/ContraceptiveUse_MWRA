...
GetCIs <- function (# Construct country-specific CIs
  ...
  for (c in 1:winbugs.data$C){
    ...
    if (!include.AR){
      ...
      p.str <- InternalGetTrajectoriesCountryTot(c = c, years.i = years.ci[c,1:winbugs.data$N.unique.c[c]], 
      ...
      #A[1:2, 1:2,1]
      #apply(A,2, cbind)  
      R.str <- InternalGetTrajectoriesCountryRat(c = c, years.i = years.ci[c,1:winbugs.data$N.unique.c[c]], 
                                                 start.year = start.year, end.year = end.year,
                                                 mcmc.array = mcmc.array.thin, 
                                                 do.country.specific.run = do.country.specific.run, # change JR, 20131104
                                                 winbugs.data = winbugs.data, # change JR, 20131104
    ...
    if (!include.AR){
      ...
      theta.str <- InternalGetARTrajectories(rho.s = rho.unmet, 
                                             ...
                                             eps.is = theta.is,
                                             years.i = years.ci[c,1:winbugs.data$N.unique.c[c]], 
                                             start.year = start.year, end.year = end.year,
                                             nrepeatARsampling = nrepeatARsampling,
