WriteCountryModel <- function(# Writes BUGS model
...
# Leontine Alkema, 2011 & Jin Rou New, 2013
#--------------------------------------------------------------
...
for (subreg in 1:n.subreg){
  ...
  T.subreg[subreg] <- T.subreg0[subreg]
...
for (j in 1:J){ 
  ...
  # change JR, 20131121 # not used for source.ind.j == 5
...
for (k in 1:n.training.modern) {    
  ...
  tau.modern.j[getj.training.modern.k[k]] <- pow(se.modern.j[getj.training.modern.k[k]], -2)
}
...
for (s in 1:4){
  ...
  T.s[s,1,1] <- T1.source.s0[s] 
  T.s[s,2,2] <- T2.source.s0[s]
  T.s[s,1,2] <- T12.source.s0[s]
  T.s[s,2,1] <- T12.source.s0[s]
}
#--------------------------------------------------------------
a.unmet <- a.unmet0
b.unmet <- b.unmet0
...
sigma.unmet.other <- sigma.unmet.other0
sigma.unmet.dhs <- sigma.unmet.dhs0
...
sigma.earlierTc <- sigma.earlierTc0
