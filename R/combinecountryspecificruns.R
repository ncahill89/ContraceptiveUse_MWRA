...
CombineCountrySpecificRuns <- function(# Combine results of country-specific runs.
  ...
  if (!file.exists(file.path(output.dir, "mcmc.meta.rda"))) {
    ...
    RunMCMC(run.name = paste0(run.name.global, "_all"),
            ...
            run.name.global = mcmc.meta$general$run.name.global,
            run.jags = FALSE,
            ...
            data.csv = data.csv, regioninfo.csv = regioninfo.csv, 
  ...
  if (get.combined.results) { # combine output of country-specific runs
    ...
    for (c in 1:C) {
      ...
      if (c != 1) {
        ...
        Zstuff.all$Zstar.cqp <- abind(Zstuff.all$Zstar.cqp, res.country$Zstuff$Zstar.cqp, along = 1)
        par.ciq.all <- abind(par.ciq.all, par.ciq, along = 1)
      }
      # combine mcmc.array
      ...
      # combine mcmc.array
      mcmc.array.all <- abind(mcmc.array.all, mcmc.array)
    } # end country loop  
    # add back Zstuff
