...
GetPosteriorProbFromCutoffProp <- function(# Calculate posterior probability given cut-off proportion
  ...
  indicator, ##<< Any of the following choices: "Total", "Modern", "Traditional", "Modern/Total",
  ...
  # change JR, 20140311
  if (indicator %in% c("Modern/Total", "Met Demand", "Met Demand with Modern Methods") & 
        is.null(cutoffprop))
    stop("cutoffprop cannot be NULL if indicator is Modern/Total, Met Demand or Met Demand with Modern Methods.")
...
GetCountryPostSamples <- function( # Get posterior samples of some indicator for a country
  ...
  year = NULL, ##<< Year to get posterior samples of, gets all years if \code{NULL}.
  indicator ##<< Any of the following choices: "Total", "Modern", "Traditional", "Modern/Total",
  ## "Unmet", "TotalPlusUnmet", "TradPlusUnmet", "Met Demand", "Met Demand with Modern Methods"
) {
  if (!(indicator %in% c("Total", "Modern", "Traditional", "Modern/Total",
                         "Unmet", "TotalPlusUnmet", "TradPlusUnmet", 
                         "Met Demand", "Met Demand with Modern Methods")))
    stop(paste0("indicator must be one of the following choices: ",
                "Total, Modern, Traditional, Modern/Total, ",
                "Unmet, TotalPlusUnmet, TradPlusUnmet, Met Demand or Met Demand with Modern Methods."))
  est.years <- dimnames(props.array)[[1]]
  if (is.null(year)) {
    select.years <- seq_along(est.years)
  } else {
    select.years <- match(paste(year), est.years)
  }
  if (indicator %in% dimnames(props.array)[[2]]) {
    samps <- props.array[select.years, indicator, ]
  } else if (indicator == "Total") {  
    samps <- props.array[select.years, "Traditional", ] + props.array[select.years, "Modern", ]
  } else if (indicator == "Modern/Total") {  
    samps <- props.array[select.years, "Modern", ]/(
      props.array[select.years, "Traditional", ] + props.array[select.years, "Modern", ])
  } else if (indicator == "TotalPlusUnmet") {
    samps <- props.array[select.years, "Traditional", ] + props.array[select.years, "Modern", ] + props.array[select.years, "Unmet", ]
  } else if (indicator == "TradPlusUnmet") {
    samps <- props.array[select.years, "Traditional", ] + props.array[select.years, "Unmet", ]
  } else if (indicator == "Met Demand") {
    samps <- (props.array[select.years, "Traditional", ] + props.array[select.years, "Modern", ])/
      (props.array[select.years, "Traditional", ] + props.array[select.years, "Modern", ] + 
         props.array[select.years, "Unmet", ])
